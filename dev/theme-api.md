...
Finder API 由 `linkFinder` `linkFeedFinder` 对象提供，可在主题模板的任意位置使用，无需依赖 `/links` 路由页面。
...
1. `groupName: string` — 链接分组名称，对应 `LinkGroupVo.metadata.name`
2. `linkName: string` - 链接名称，对应 `LinkVo.metadata.name`
3. `beforePublishedAt: string` -  下标发布时间
4. `beforeId: string` - 下标ID
5. `limit: int` - 最大条数
